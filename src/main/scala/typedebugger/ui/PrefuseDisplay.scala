...
abstract class PrefuseDisplay(source0: io.AbstractFile, t: Tree, vis: TypeDebuggerVisualization)(implicit idx: Int)
  ...
  class CustomTreeLayout() extends VisibilityAwareNodeLinkTreeLayout(orientation, 50, 0, 8, dataGroupName, visibleGroup) {
    def realRoot(): NodeItem = getBottomNode
    def isCustomNodeVisible(node: NodeItem) = !adv.isAdvancedOption(node) || adv.isOptionEnabled(node)
    def allOpenGoals() = m_vis.getFocusGroup(openGoalNodes)
  }
